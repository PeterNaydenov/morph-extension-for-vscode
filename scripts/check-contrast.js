...
function validateTheme(themePath) {
  ...
  theme.tokenColors?.forEach((token) => {
    if (token.settings.foreground) {
      results.push(
        checkContrast(token.settings.foreground, bg, `${token.name || 'token'}`)
      );
    }
  });
...
function main() {
  ...
  themes.forEach((themePath) => {
    ...
    results.forEach((result) => {
      ...
      console.log(
        `${status} ${result.element}: ${result.ratio}:1 (AA: ${status} AAA: ${aaaStatus})`
      );

      if (!result.passesAA) {
        allPassed = false;
        console.log(
          `  ❌ FAIL: ${result.foreground} on ${result.background} - Ratio ${result.ratio} < 4.5:1`
        );
      }
    });
  });
